...
AWS Lambda entry point para auditar stages de API Gateway que no tienen
logs de acceso ni X-Ray tracing habilitado.

Variables de entorno:
  REPORTS_BUCKET   – S3 bucket donde se almacenan los reportes.

Layout de salida en S3:
  reports/audit_apigateway_logs/<account_id>/<timestamp>.json
...
def _s3_key_for_report(timestamp_str: str, account_id: str) -> str:
    ...
    return f"reports/audit_apigateway_logs/{account_id}/{safe_ts}.json"
...
def lambda_handler(event: dict, context: object) -> dict:
    ...
    logger.info("Iniciando auditoría API Gateway Logs & Tracing — account=%s ts=%s", account_id, timestamp_str)
