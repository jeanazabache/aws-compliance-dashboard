...
def _s3_key_for_report(timestamp_str: str, account_id: str) -> str:
    ...
    return f"reports/audit_apigateway_waf/{account_id}/{safe_ts}.json"
...
def lambda_handler(event: dict, context: object) -> dict:
    ...
    logger.info("Iniciando auditoría API Gateway WAF — account=%s ts=%s", account_id, timestamp_str)

    audit_results = run_audit(account_id)
