...
Lambda entry point para auditar el sidecar Fluent Bit en servicios ECS.
Escribe el reporte al bucket central con prefijo por cuenta:

  reports/audit_ecs_fluentbit/<account_id>/<timestamp>.json
...
def _s3_key_for_report(timestamp_str: str, account_id: str) -> str:
    ...
    return f"reports/audit_ecs_fluentbit/{account_id}/{safe_ts}.json"
...
def lambda_handler(event: dict, context: object) -> dict:
    ...
    logger.info("Iniciando auditoría ECS Fluent Bit — account=%s ts=%s", account_id, timestamp_str)
    ...
    summary = report["summary"]
    logger.info(
        "Listo — services=%d compliant=%d non_compliant=%d skipped=%d clusters=%d",
        summary["total"],
        summary["compliant"],
