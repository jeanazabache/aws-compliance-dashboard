...
AWS Lambda entry point para auditar distribuciones de CloudFront que no tienen
...
Layout de salida en S3:
  reports/audit_cloudfront_waf/<account_id>/<timestamp>.json
...
def _s3_key_for_report(timestamp_str: str, account_id: str) -> str:
    ...
    return f"reports/audit_cloudfront_waf/{account_id}/{safe_ts}.json"
...
def lambda_handler(event: dict, context: object) -> dict:
    ...
    logger.info("Iniciando auditoría CloudFront WAF — account=%s ts=%s", account_id, timestamp_str)
