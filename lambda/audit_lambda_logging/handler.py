...
AWS Lambda entry point para auditar funciones Lambda que no pueden
registrar logs en CloudWatch (por DENY explícito o ausencia de permisos).

Variables de entorno:
  REPORTS_BUCKET   – S3 bucket donde se almacenan los reportes.

Layout de salida en S3:
  reports/audit_lambda_logging/<account_id>/<timestamp>.json
  reports/index.json (manifest acumulativo)

Programación recomendada: rate(4 hours)
"""
...
def _s3_key_for_report(timestamp_str: str, account_id: str) -> str:
    ...
    return f"reports/audit_lambda_logging/{account_id}/{safe_ts}.json"
...
def lambda_handler(event: dict, context: object) -> dict:
    ...
    logger.info("Iniciando auditoría Lambda Logging — account=%s ts=%s", account_id, timestamp_str)
    ...
    summary = report["summary"]
    logger.info(
        "Listo — total=%d habilitado=%d deshabilitado=%d",
        summary["total"],
        summary["habilitado"],
