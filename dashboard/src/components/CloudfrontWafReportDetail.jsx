...
export default function CloudfrontWafReportDetail({ report }) {
  const [pageSize, setPageSize] = useState(25);
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState("all");
  const [wafFilter, setWafFilter] = useState("all");
  ...
  const filtered = useMemo(() => {
    ...
    return results
      ...
      .filter((r) => wafFilter === "all" || r.waf_web_acl_name === wafFilter)
      .filter(
        (r) =>
          !term ||
          r.distribution_id.toLowerCase().includes(term) ||
  ...
  }, [results, statusFilter, wafFilter, search]);
  ...
  return (
    <div>
      ...
      <div style={styles.reportHeader}>
        <div>
          <div style={styles.reportTitle}>CloudFront — Protección WAF</div>
      ...
      <div style={styles.cards}>
        ...
        />
        <SummaryCard
          value={(summary.enabled ?? 0).toLocaleString()}
          label="Habilitadas"
          color="var(--accent-2)"
        />
      </div>
      ...
      {/* WAF ACLs in use */}
      {waf_acls.length > 0 && (
        <Panel title="Web ACLs en uso">
          <div style={styles.tableWrap}>
            <table style={styles.table}>
              <thead>
                <tr>
                  {["Web ACL", "Distribuciones protegidas"].map((h) => (
              ...
              <tbody>
                {waf_acls.map((acl) => {
                  const active = wafFilter === acl.name;
                  return (
                    <tr
                      key={acl.name}
                      style={{ ...styles.trHover, cursor: "pointer" }}
                      onClick={() => { setWafFilter(active ? "all" : acl.name); setPage(1); }}
                      title="Click para filtrar por este Web ACL"
                    >
                      <td style={{ ...styles.td, fontWeight: 600 }}>
                        🛡️ {acl.name}
                        {active && <span style={styles.activeFilterBadge}>filtrado</span>}
                      </td>
                      <td style={styles.td}>{acl.distributions}</td>
                    </tr>
                  );
                })}
              </tbody>
      ...
      <div style={styles.toolbar}>
        <div style={styles.filterGroup}>
          ...
          {wafFilter !== "all" && (
            <button
              onClick={() => { setWafFilter("all"); setPage(1); }}
              style={styles.clearFilterBtn}
              title="Quitar filtro de WAF"
            >
              <span>{wafFilter} · {filtered.length}</span>
        ...
        <input
          ...
          placeholder="🔍 Buscar distribución, dominio o alias…"
          value={search}
      ...
      <div style={styles.tableWrap}>
        <table style={styles.table}>
          <thead>
            <tr>
              {["Distribución", "Dominio / Aliases", "Comentario", "Estado CF", "WAF (Web ACL)", "Cumplimiento"].map((h) => (
                <th key={h} style={styles.th}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {pageItems.length === 0 ? (
              ...
              pageItems.map((r) => (
                <tr key={r.distribution_id} style={styles.trHover}>
                  <td style={{ ...styles.td, fontFamily: "ui-monospace, monospace", fontSize: 12, fontWeight: 600 }}>
                    {r.distribution_id}
                  </td>
                  <td style={styles.td}>
                    <div style={{ fontSize: 12 }}>{r.domain_name}</div>
                    {r.aliases && r.aliases.length > 0 && (
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: 4 }}>
                        {r.aliases.map((alias) => (
                          <span key={alias} style={styles.aliasTag}>{alias}</span>
                        ))}
                      </div>
                    )}
                  </td>
                  <td style={{ ...styles.td, fontSize: 12, color: "var(--muted)", maxWidth: 200 }}>
                    {r.comment || "—"}
                  </td>
                  <td style={styles.td}>
                    <span style={{
                      ...styles.statusTag,
                      color: r.enabled ? "var(--green)" : "var(--muted)",
                    }}>
