...
export default function ApigatewayLogsReportDetail({ report }) {
  ...
  const filtered = useMemo(() => {
    ...
    return results
      ...
      .filter((r) => typeFilter === "all" || r.api_type === typeFilter)
      .filter((r) => {
        if (featureFilter === "all") return true;
        if (featureFilter === "access_log") return r.has_access_logging;
        if (featureFilter === "no_access_log") return !r.has_access_logging;
        if (featureFilter === "tracing") return r.has_tracing;
        if (featureFilter === "no_tracing") return !r.has_tracing;
        return true;
      })
      .filter(
        (r) =>
          !term ||
          r.api_name.toLowerCase().includes(term) ||
          r.api_id.toLowerCase().includes(term) ||
          (r.detail || "").toLowerCase().includes(term),
      );
  }, [results, statusFilter, regionFilter, typeFilter, featureFilter, search]);
  ...
  return (
    <div>
      ...
      <div style={styles.reportHeader}>
        <div>
          <div style={styles.reportTitle}>API Gateway — Logs &amp; Tracing</div>
      ...
      <div style={styles.cards}>
        ...
        <SummaryCard
          ...
          label="Sin logs/tracing"
          color="var(--red)"
          active={statusFilter === "Non-compliant"}
          onClick={() => toggleStatus("Non-compliant")}
        />
        <SummaryCard
          value={(summary.with_access_log ?? 0).toLocaleString()}
          label="Access Log"
          color="#0891b2"
          active={featureFilter === "access_log"}
          onClick={() => { setFeatureFilter(featureFilter === "access_log" ? "all" : "access_log"); setPage(1); }}
        />
        <SummaryCard
          value={(summary.with_tracing ?? 0).toLocaleString()}
          label="X-Ray Tracing"
          color="#7c3aed"
          active={featureFilter === "tracing"}
          onClick={() => { setFeatureFilter(featureFilter === "tracing" ? "all" : "tracing"); setPage(1); }}
        />
      </div>
      ...
      <div style={styles.toolbar}>
        <div style={styles.filterGroup}>
          ...
          {typeFilter !== "all" && (
            <button onClick={() => { setTypeFilter("all"); setPage(1); }} style={styles.clearFilterBtn}>
              <span>{typeFilter}</span><span style={styles.clearX}>✕</span>
            </button>
          )}
        </div>
        <input
          style={styles.searchInput}
          placeholder="🔍 Buscar API o detalle…"
          value={search}
      ...
      <div style={styles.tableWrap}>
        <table style={styles.table}>
          <thead>
            <tr>
              {["API", "Tipo", "Stage", "Región", "Access Log", "Exec Log", "X-Ray"].map((h) => (
          ...
          <tbody>
            {pageItems.length === 0 ? (
              ...
              pageItems.map((r) => (
                <tr key={`${r.region}::${r.api_id}::${r.stage_name}`} style={styles.trHover}>
                  <td style={styles.td}>
                    <div style={{ fontWeight: 600 }}>{r.api_name}</div>
                    <div style={{ fontSize: 11, color: "var(--muted)", fontFamily: "ui-monospace, monospace" }}>
                      {r.api_id}
                    </div>
                  </td>
                  <td style={styles.td}>
                    <span style={styles.typeTag}>{r.api_type}</span>
                  </td>
                  ...
                  <td style={styles.td}>
                    <BoolBadge value={r.has_access_logging} />
                  </td>
                  <td style={styles.td}>
                    {r.api_type === "REST" ? (
                      <span style={{ fontSize: 11, color: r.has_execution_logging ? "var(--green)" : "var(--red)", fontWeight: 600 }}>
                        {r.execution_log_level || "OFF"}
                      </span>
                    ) : (
...
function BoolBadge({ value }) {
  return (
    <span style={{
      ...
      padding: "2px 8px",
      borderRadius: 4,
      background: value ? "var(--green-soft, #d1fae5)" : "var(--red-soft, #fee2e2)",
      color: value ? "#047857" : "#b91c1c",
      border: `1px solid ${value ? "#6ee7b7" : "#fecaca"}`,
    }}>
      {value ? "✓ Sí" : "✗ No"}
    </span>
  );
...
function Pill({ label, accent, color }) {
  const resolved = color
    ...
    : { background: accent ? "var(--accent-soft)" : "var(--surface-2)", color: accent ? "var(--accent)" : "var(--muted)", border: `1px solid ${accent ? "transparent" : "var(--border)"}` };
...
const styles = {
  ...
  typeTag: { fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 4, background: "var(--surface-2)", border: "1px solid var(--border)", color: "var(--text-2)" },
