...
export default function ApigatewayWafReportDetail({ report }) {
    ...
    return results
      ...
      .filter(
        (r) =>
          ...
          (r.waf_web_acl_name || "").toLowerCase().includes(term),
      );
  }, [results, statusFilter, regionFilter, typeFilter, search]);
  ...
  return (
    <div>
      ...
      <div style={styles.reportHeader}>
        <div>
          <div style={styles.reportTitle}>API Gateway — Protección WAF</div>
      ...
      {/* Region breakdown */}
      {regions.length > 0 && (
        <Panel title="Cobertura por región">
          <div style={styles.tableWrap}>
            <table style={styles.table}>
              <thead>
                <tr>
                  {["Región", "Total", "Con WAF", "Sin WAF", "Cobertura"].map((h) => (
              ...
              <tbody>
                {regions.map((r) => {
                  ...
                  const active = r.region === regionFilter;
                  return (
                    <tr
                      key={r.region}
                      style={{ ...styles.trHover, cursor: "pointer" }}
                      onClick={() => { setRegionFilter(active ? "all" : r.region); setPage(1); }}
                      title="Click para filtrar por esta región"
                    >
                      ...
                      <td style={{ ...styles.td, color: "var(--green)", fontWeight: 600 }}>{r.compliant}</td>
                      <td style={{ ...styles.td, color: r.non_compliant > 0 ? "var(--red)" : "var(--muted)", fontWeight: 600 }}>
                        {r.non_compliant}
                      </td>
      ...
      {Object.keys(by_type).length > 0 && (
        <Panel title="Por tipo de API">
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            {Object.entries(by_type).map(([type, counts]) => {
              const pct = counts.total ? Math.round((counts.compliant / counts.total) * 100) : 0;
              const active = typeFilter === type;
              return (
                <button
                  key={type}
                  type="button"
                  onClick={() => { setTypeFilter(active ? "all" : type); setPage(1); }}
                  style={{
                    ...styles.typeCard,
                ...
                >
                  ...
                  <span style={{ fontSize: 12, color: "var(--muted)" }}>
                    {counts.total} stages · {pct}% con WAF
                  </span>
                </button>
              );
            })}
      ...
      <div style={styles.toolbar}>
        <div style={styles.filterGroup}>
          ...
          {regionFilter !== "all" && (
            <button
              ...
              title="Quitar filtro de región"
            >
              <span>{regionFilter} · {filtered.length}</span>
          ...
          {typeFilter !== "all" && (
            ...
            >
              <span>{typeFilter} · {filtered.length}</span>
              <span style={styles.clearX}>✕</span>
            </button>
          )}
        </div>
        ...
        <input
          ...
          placeholder="🔍 Buscar API, ID o WAF…"
          value={search}
      ...
      <div style={styles.tableWrap}>
        <table style={styles.table}>
          <thead>
            <tr>
              {["API", "Tipo", "Stage", "Región", "WAF (Web ACL)", "Estado"].map((h) => (
                <th key={h} style={styles.th}>{h}</th>
              ))}
            </tr>
          </thead>
                    ...
                    {r.has_waf ? (
                      <div>
                        <div style={{ fontWeight: 600, color: "var(--green)" }}>
                          🛡️ {r.waf_web_acl_name}
                        </div>
                        <div style={{ fontSize: 11, color: "var(--muted)", fontFamily: "ui-monospace, monospace", wordBreak: "break-all" }}>
                          {r.waf_web_acl_id}
                        </div>
                      </div>
                    ) : (
                      <span style={{ color: "var(--red)", fontWeight: 600 }}>⚠️ Sin WAF</span>
                    )}
                  </td>
                  <td style={styles.td}>
